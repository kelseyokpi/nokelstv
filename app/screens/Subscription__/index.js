...
const SubscriptionView = ({ navigation, route }) => {
    ...
    const _renderItems = (item, index) => {
        return (
            <TouchableOpacity
                style={{
                    backgroundColor: SelectedPlan === index ? PLAN_ACTIVE_COLOR_CODE : PLAN_DEACTIVE_COLOR_CODE,
                ...
                <Text style={{
                    fontFamily: FONT_FAMILY_BOLD, fontSize: 15,
    ...
    const _handleSuccessPayment = async (response, CommonId) => {
        ...
        const params = {
            plan_id: Item.id,
            transaction_id: CommonId === 0 ? response.response.id : response.transaction,
            payment_detail: '',
            amount: finalamount,
            ...
            <Dialog
                ...
                dialogTitle={
                    <View style={{ alignItems: "center", width: '100%', paddingTop: 20, paddingBottom: 10 }}>
                        <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: '#636363', fontSize: 16 }}>Select your payment gateway</Text>
                    </View>
                }
            >
                <DialogContent>
                    <View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 10 }}>
                            <View style={{ alignItems: 'center' }}>
                                ...
                                <TouchableOpacity onPress={() => onPressPaystack(1)}>

                                    {SelectPayment === 1 ?
                                        <Image source={require('../../assets/icon-checkbox-checked.png')} /> :
                                        <Image source={require('../../assets/icon-checkbox.png')} />
                                    }
                                </TouchableOpacity>
                            </View>
                            <View style={{ paddingTop: 10, alignItems: 'center' }}>
                                <Image style={{ width: 110, height: 60 }} source={require('../../assets/paypal_mb8k.png')} />
                                <TouchableOpacity onPress={() => onPressPaystack(2)}>
                                    {SelectPayment === 2 ?
                                        <Image style={{ top: 10 }} source={require('../../assets/icon-checkbox-checked.png')} /> :
                                        <Image style={{ top: 10 }} source={require('../../assets/icon-checkbox.png')} />
                                    }
                                </TouchableOpacity>
                        ...
                        <TouchableOpacity
                            ...
                            style={[styles.confirmBtnSTyle, { backgroundColor: SelectPayment === "" ? '#e6e6e6' : COMMONTHEMECOLOR, marginTop: 20 }]}
                        >
                            <Text style={{ fontFamily: FONT_FAMILY_REGULAR, fontSize: 16, color: SelectPayment === "" ? BLACK_COLOR_CODE : WHITE_COLOR_CODE }}>Confirm</Text>
                        </TouchableOpacity>
                    </View>
                </DialogContent>
            </Dialog>
