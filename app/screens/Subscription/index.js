...
const SubscriptionView = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (products?.length > 0) {
      ...
      requestSubscription(products[0].productId)
    }
  ...
  useEffect(() => {
    ...
    const checkCurrentPurchaseE = async (error?: error): Promise<void> => {
      console.log('error: ', error);
    };
    checkCurrentPurchase(currentPurchase);
    checkCurrentPurchaseE(currentPurchaseError)
  ...
  async function _handleSubscription(){
    ...
    try{
      const params = {
        ...
        'amount':finalamount,
        'paystrack_plan_id':  Item.paystrack_plan_id,
      }
      
     console.log('params: ', params);
    const { data } = await apiCall("POST", ENDPOINTS.CREATESUBSCRIPTIONMOBILE,params);
    console.log('data: ', data);
    if (data.status === 200) {
      setVisible(false);
        Linking.openURL(data.data.data.authorization_url);
  ...
  function NavToSignUp() {
    ...
    //  Item.id == 0 ? _handleSuccessPayment(0, 3) : setShowPayment(true);
      Item.id == 0 ? _handleSuccessPayment(0, 3) :_handleSubscription();
      //setShowPayment(true);
    } else {
  ...
  async function handleInAppPurchase() {
    ...
    if (Item.id == 0) {
      ...
      console.log('producthandleInAppPurchase,: ', products)
      // .then((res) => {
      // console.log('res: ', res);
  ...
  return (
    <View style={{ flex: 1 }}>
      {
        console.log('productsreturn: ', products)
      }
      {
